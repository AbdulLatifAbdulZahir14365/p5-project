...
// Function begins the game by calling to setup the start screen
function setup() {
  setupStart();
  setLineDash([5, 5]);
}
...
// Function establishes the start screen
function setupStart() {
  background(0);
...
function draw() {
  ...
  if (state == "gameMenu") {
    ...
    if (ballPosX <= 80) {
      ...
      (ballBottom >= 710 && ballDirectionY > 0) ||
      (ballTop <= 15 && ballDirectionY < 0)
    ) {
      ballDirectionY *= -1;
      pongSound.play();
      randomizeColor();
      // Tracks whether the ball has hit the player paddle
    } else if (
      ballLeft <= myRight &&
      ballPosY >= myPosY - 50 &&
      ballPosY <= myPosY + 50 &&
